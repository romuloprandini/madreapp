...
namespace MadreApp.iOS.Renderers
{
    public class CircularProgressBarRenderer : ViewRenderer
    {
        ...
        CircularProgress nativeProgress;
        protected override void OnElementChanged(ElementChangedEventArgs<Xamarin.Forms.View> e)
        {
            ...
            var frame = new CGRect(0, 301, Frame.Width, Frame.Height / 2);
            nativeProgress = new CircularProgress(frame, true);

            var center = new CGPoint(160, 200);
            var bezier = new UIBezierPath();
            bezier.AddArc(center, nativeProgress.Frame.Width / 3, (nfloat)Math.PI, (nfloat)0.0, true);
            nativeProgress.Path = bezier;

            nativeProgress.Colors = new[]
            {
                ColorExtension.RgbaColor(0xDC1B66AA).CGColor
            };

            nativeProgress.LineWidth = 10;
            nativeProgress.ProgressGuideColor = ColorExtension.RgbaColor(0x999999AA);

            SetNativeControl(nativeProgress);
        }
        ...
        protected override void OnElementPropertyChanged(object sender, PropertyChangedEventArgs e)
        {
            ...
            if (e.PropertyName == "Progress")
            {
                ...
                {
                    nativeProgress.Progress = progressBar.Progress;
                }
                else
                {
                    nativeProgress.Progress = 0;
                }
            }
        }
